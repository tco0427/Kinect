...
namespace KinectModule
{
    ...
    /// </summary>
    public delegate Skeleton[] SkeletonSelectFunc(Skeleton[] s);
    ...
    /// <returns>선별된 스켈레톤 데이터</returns>
    /// </summary>
    public delegate Dictionary<int, XSkeleton> SkeletonCorrectFunc(Skeleton[] s);
    ...
    public class XJoint : ICloneable
    {
        ...
        public static XJoint operator +(XJoint lData, XJoint rData)
        {
            XJoint result = new XJoint();
            result.x = lData.x + rData.x;
            result.y = lData.y + rData.y;
            result.z = lData.z + rData.z;
            return result;
        }
        ...
        public static XJoint operator /(XJoint lData, double rData)
        {
            XJoint result = new XJoint();
            if (rData != 0)
            {
                result.x = lData.x / rData;
                result.y = lData.y / rData;
                result.z = lData.z / rData;
            }
    ...
    public class XSkeleton
    {
        ...
        public static implicit operator XSkeleton(byte[] source)
        {
            ...
            if (source.Length < 20 * 3 * sizeof(double))
                return value;

            for (int partNum = 0; partNum < 20; partNum++)
            {
        ...
        public static implicit operator XSkeleton(Skeleton source)
        {
            XSkeleton value = new XSkeleton();
        ...
        {
            byte[] value = new byte[20 * 3 * sizeof(double)];
            for (int partNum = 0; partNum < 20; partNum++)
            {
    ...
    public class PoseValue : ActionValue
    {
        ...
        public static implicit operator PoseValue(byte[] value)
        {
            ...
            //Skeleton
            byte[] _skelBuf = new byte[20 * sizeof(double) * 3];
    ...
    public class UserInformation
    {
        ...
        public static implicit operator UserInformation(byte[] value)
        {
            ...
            //골격정보
            byte[] _skelBuf = new byte[sizeof(double) * 3 * 20];
            Array.Copy(value, _startPos, _skelBuf, 0, _skelBuf.Length);
    ...
    public struct InputMessage
    {
        ...
        public InputMessage(string deviceName, int userID, int usePosition, Point position, int pointCount, ValuePair<int, Point>[] points, int eventType, double eventValue, int uiID)
        {
        ...
        public static implicit operator byte[](InputMessage ui)
        {
            ...
            //총 길이
            int _totalLen = sizeof(int) + _nameLen.Length + _deviceName.Length + _id.Length + _usePosition.Length + _position.Length + _pointCount.Length + _pointsLen + _event.Length + _eventValue.Length + _uiID.Length;
            byte[] _total = BitConverter.GetBytes(_totalLen);
